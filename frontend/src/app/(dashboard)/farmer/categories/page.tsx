  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f8f9fa]">
        <div className="w-12 h-12 border-4 border-[#2d9a33] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#fafafa]">
      <header className="h-20 bg-white border-b border-gray-100 flex items-center px-8 flex-shrink-0 z-10 sticky top-0">
        <h1 className="text-2xl font-black text-gray-900 tracking-tight" style={{ fontFamily: 'Impact, Arial Black, sans-serif' }}>
          Product Categories
        </h1>
      </header>
              ...
              <tbody className="divide-y divide-gray-100">
                {categories.length === 0 ? (
                  <EmptyTableState
                    ...
                    description="Product categories will appear here once they are configured on the platform."
                  />
                ) : categories.map((cat) => (
